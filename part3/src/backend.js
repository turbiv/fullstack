...
app.use(
  morgan(
    ":method :url :status :res[content-length] - :response-time ms :persondetails"
  )
);
...
app.post("/api/persons", (request, response, next) => {
  ...
  morgan.token("persondetails", (req, res) =>
    req.method === "POST" ? JSON.stringify(newPerson) : false
  );
...
const errorHandler = (error, request, response, next) => {
  if (error.name === "CastError" && error.kind === "ObjectId") {
    ...
    (error.errors.number.properties.type === "unique" ||
      error.kind === "unique")
  ) {
